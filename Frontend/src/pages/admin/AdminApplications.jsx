...
export default function AdminApplications() {
  ...
  const handleUpdate = async () => {
    if (selected?.status === "ALLOCATED" && form.status !== "ALLOCATED") {
      const shouldDeallocate = window.confirm("This will deallocate the room from the student. Continue?");
      if (!shouldDeallocate) return;
    }

    if (form.status === "ALLOCATED") {
      const shouldAllocate = window.confirm("Allocate this room to the student?");
    ...
    try {
      ...
      await api.put(`/rooms/admin/applications/${selected.id}`, updateData);
